...
public:
	...
	void create(const string& table_directory,const string& table_name_,list<string> columns);
    //调用open返回一个TableRow对象，实现对二进制文件的操作
    string open(const string& table_name);
    //对表头文件和二进制文件进行删除
    void drop(const string& table_name);
	int Get_columns_num(const string& table_name_);
};
#endif
/*
{
